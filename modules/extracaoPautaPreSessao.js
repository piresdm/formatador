...
function isLinhaAdvogado(linha) {
  return /^[-–—:;\s]*adv\./i.test(linha);
}

function limparLinhaAdvogado(linha) {
  return removerParenteses(linha)
    .replace(/^[-–—:;\s]*adv\.\s*[:;-]?\s*/i, "")
    .trim();
}
...
function extrairProcessosDoc2(html) {
  ...
  linhas.forEach((tr) => {
    ...
    linhasCol2.slice(1).forEach((linha) => {
      ...
      if (isLinhaAdvogado(linhaLimpa)) {
        const advogadoLimpo = limparLinhaAdvogado(linhaLimpa);
